...
function renderProductStatsChart(data,labels) {
    // Main chart
    var ctxL = document.getElementById("productStats").getContext('2d');
    var myLineChart = new Chart(ctxL, {
        type: 'line',
        data: {
            labels: labels,
...
function productStatsChart() {
    var url = $('[name="productStatsDataUrl"]').val();
    var data = {};
    $.get(url, data, function (response) {
        response = JSON.parse(response);
        if (response.status) {
            renderProductStatsChart(response.dataset, response.labels)
        }
    });
